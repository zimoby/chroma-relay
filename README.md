...
The local CEP link is `~/Library/Application Support/Adobe/CEP/extensions/com.zimoby.chroma-relay` and points to `dist/cep`.

In After Effects 2022 or later, open **Window → Extensions → Chroma Relay**. Open the hidden `com.zimoby.chroma-relay.settings` surface from Main's panel flyout. Main and Settings use CDP ports 8198 and 8199.

Production builds omit `window.__CHROMA_RELAY_DEBUG__`. Build and inspect the development contract with:
...
npm run cdp:mutate -- --output=evidence/local/mutation-smoke
npm run cdp:collect -- --output=evidence/local/host-smoke
npm run cdp:apply -- --output=evidence/local/apply-smoke
```

Capture responsive geometry/state fixtures with `npm run cdp:design -- --output=evidence/local/responsive`. The real Main compositor must be at least 200×200 first; the runner fails rather than saving malformed evidence from a smaller compositor surface.
...
- Main ID: `com.zimoby.chroma-relay.main`.
...
- At the measured 132 px internal CEP viewport floor, vertical Stretch uses a left-aligned 24×24 px Add fallback so an already undersized host frame cannot clip the control.
- Dragging reorders colors in Main. Alt/Option-click removes directly; the compact × control exposes explicit remove mode for pointer or keyboard use.
- Main and Settings use one continuous AE-native background; Settings uses compact General/Palettes tabs and a low-priority status line.
- Production packages omit debug endpoints, source maps, and destructive context-menu actions.

## Implementation status

- [x] I01–I11 are implemented and validated in live AE 26.3.
- [x] Flexible collection, one-click removal, transient notices, disabled-branch settings, and fixed Add sizing are live-validated.
...
- `docs/demo-implementation-plan.md` — disposable two-panel, JSON-persisted, no-automated-testing path for proving the core interaction as quickly as possible.
- `docs/implementation-plan.md` — canonical from-scratch rebuild sequence with failure-prevention gates, current reconciliation, and the original plan preserved as a historical appendix.
- `docs/implementation-status.md` — milestone checklist, validation, blockers, and exact implementation file inventory.
- `docs/design-direction.md` — selected visual direction.
